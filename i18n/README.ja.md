# GenRTL MCP - どんなプロンプトにも最新のコードドキュメントで応える
...
GenRTL MCP は最新のバージョン固有のドキュメントとコード例をソースから直接取得し、プロンプトに直接配置します。
Cursor のプロンプトに `use genrtl` を追加するだけ：
...
GenRTL は最新のコード例とドキュメントを直接 LLM のコンテキストに取得します。

- 1️⃣ 普段通りにプロンプトを書く
- 2️⃣ LLM に `use genrtl` と指示する
- 3️⃣ 動作するコードの回答を得る
  タブの切り替えも、存在しない API のハルシネーションも、古いコード生成もありません。

## 📚 プロジェクトの追加

[プロジェクト追加ガイド](https://genrtl.com/docs/adding-libraries) をチェックして、お気に入りのライブラリを GenRTL に追加（または更新）する方法を学びましょう。
...
[Smithery](https://smithery.ai/server/@upstash/genrtl-mcp) 経由で任意のクライアントに GenRTL MCP サーバーを自動的にインストールするには：

```bash
npx -y @smithery/cli@latest install @upstash/genrtl-mcp --client <CLIENT_NAME> --key <YOUR_SMITHERY_KEY>
```

Smithery キーは [Smithery.ai Web ページ](https://smithery.ai/server/@upstash/genrtl-mcp) で確認できます。
...
3. **リモートサーバー** タブを選択します。
4. **設定を編集** ボタンをクリックします。
5. genrtl に関連する設定を `mcpServers` に追加します：
```json
{
  "mcpServers": {
    "genrtl": {
...
[Visual Studio MCP サーバードキュメント](https://learn.microsoft.com/visualstudio/ide/mcp-servers?view=vs-2022) に従って、Visual Studio 2022 で GenRTL MCP を設定できます。
...
[Zed Extensions](https://zed.dev/extensions?query=GenRTL) 経由でインストールできるか、Zed の `settings.json` にこれを追加できます。詳細は [Zed Context Server ドキュメント](https://zed.dev/docs/assistant/context-servers) を参照してください。
```json
{
  "context_servers": {
    "GenRTL": {
...
```
保存後、チャットで `query-docs` の後に GenRTL ドキュメント ID を入力します（例：`query-docs /nuxt/ui`）。詳細情報は [BoltAI ドキュメンテーションサイト](https://docs.boltai.com/docs/plugins/mcp-servers) で利用可能です。iOS 版 BoltAI については、[このガイドを参照してください](https://docs.boltai.com/docs/boltai-mobile/mcp-servers)。
</details>
...
1. **Docker イメージのビルド：**
   ...
   次に、タグ（例：`genrtl-mcp`）を使用してイメージをビルドします。**Docker Desktop（または Docker デーモン）が実行中であることを確認してください。** `Dockerfile` を保存した同じディレクトリで次のコマンドを実行します：
   ```bash
...
Augment Code で GenRTL MCP を設定するには、グラフィカルインターフェースまたは手動設定のいずれかを使用できます。
...
5. 以下のコマンドを入力します：
   ...
   ```
6. MCP に **GenRTL** と名前を付けます。
7. **Add** ボタンをクリックします。
MCP サーバーが追加されたら、Augment Code 内で GenRTL の最新コードドキュメンテーション機能を直接使用できます。
---
...
Zencoder で GenRTL MCP を設定するには、以下の手順に従います：
...
## 🔨 利用可能なツール
GenRTL MCP は LLM が使用できる以下のツールを提供します：
- `resolve-library-id`：一般的なライブラリ名を GenRTL 互換のライブラリ ID に変換します。
  - `query`（必須）：ユーザーの質問またはタスク（関連性によるランキングに使用）
  - `libraryName`（必須）：検索するライブラリの名前
- `query-docs`：GenRTL 互換のライブラリ ID を使用してライブラリのドキュメントを取得します。
  - `libraryId`（必須）：正確な GenRTL 互換のライブラリ ID（例：`/mongodb/docs`、`/vercel/next.js`）
...
### CLI 引数
`genrtl-mcp` は以下の CLI フラグを受け付けます：
...
## ⚠️ 免責事項
GenRTL プロジェクトはコミュニティが貢献しているもので、高品質を維持するよう努めていますが、すべてのライブラリドキュメントの正確性、完全性、セキュリティを保証することはできません。GenRTL にリストされているプロジェクトは、GenRTL ではなく、それぞれの所有者によって開発および保守されています。疑わしい、不適切な、または潜在的に有害なコンテンツを発見した場合は、プロジェクトページの「報告」ボタンを使用して、すぐにお知らせください。私たちはすべての報告を真剣に受け止め、プラットフォームの整合性と安全性を維持するために、フラグが付けられたコンテンツを迅速にレビューします。GenRTL を使用することにより、あなたは自己の裁量とリスクで使用することを認めます。

## 🤝 私たちとつながる
最新情報を入手し、コミュニティに参加しましょう：
- 📢 最新ニュースとアップデートのために [X](https://x.com/contextai) でフォローしてください
- 🌐 [Web サイト](https://genrtl.com) を訪問してください
