...
GenRTL supports all MCP clients. Below are configuration examples for popular clients. If your client isn't listed, check its documentation for MCP server installation. To configure manually, use the GenRTL server URL `https://mcp.genrtl.com/mcp` with your MCP client and pass your API key via the `GENRTL_API_KEY` header.

<Tip>
  Looking for the easiest way to get started? Use `npx grtl setup` to configure GenRTL
  automatically, and `npx grtl remove` to remove the generated setup later. See the [CLI
...
```sh
codex mcp add genrtl -- npx -y @upstash/genrtl-mcp --api-key YOUR_API_KEY
```
...
| **MCP Server URL** | `https://mcp.genrtl.com/mcp/oauth`                                                        |

> Accept the security notice and complete the one-time OAuth authorization.

#### 3. Use in a Conversation

Start a new chat → Click the plus icon → Hover over `More` → Select the `GenRTL` app.

Alternatively, you can say `use genrtl` in your prompt and ChatGPT will automatically use the `GenRTL` app.
...
Open the ChatGPT desktop app → Start a new chat → ChatGPT will automatically use the `GenRTL` app when you ask it to.
...
```sh
hermes mcp add genrtl --url https://mcp.genrtl.com/mcp/oauth --auth oauth
```
...
<Note>
Start with the API key method if you want the most direct setup. Hermes also supports OAuth for remote MCP servers via `auth: oauth`. On headless setups, Hermes may print an authorization URL to stdout or logs for you to open manually. If `hermes mcp add ... --auth oauth` drops into the interactive agent without saving the server entry, add the `mcp_servers.genrtl` config manually, then run `hermes mcp test genrtl` to trigger the auth flow and verify the connection.
</Note>
...
Use these alternatives to run the local GenRTL MCP server with other runtimes.
...
```sh
qwen mcp add --transport http genrtl https://mcp.genrtl.com/mcp \
  --header "GENRTL_API_KEY: YOUR_API_KEY" \
...
```sh
qwen mcp add genrtl npx -y @upstash/genrtl-mcp --api-key YOUR_API_KEY
```
...
- Per-workspace enable: The GenRTL button in the ProviderBar

**What you still need to do:**
Configure your coding agent (Codex, Claude Code, Cursor, etc.) to connect to GenRTL MCP. Emdash does not modify your agent's config.
...
Install the [genrtl.mcpb](https://github.com/upstash/genrtl/tree/master/mcpb/genrtl.mcpb) file and add it to your client. See [MCP bundles docs](https://github.com/anthropics/mcpb#mcp-bundles-mcpb) for more info.
