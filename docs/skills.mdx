---
title: Skills
description: Install, search, and manage AI coding assistant skills from the GenRTL registry
---
...
GenRTL maintains a registry of skills at [genrtl.com/skills](https://genrtl.com/skills) that can be installed directly into your AI coding assistant with a single command.
...
## The GenRTL Skills Registry

The [GenRTL Skills Registry](https://genrtl.com/skills) is a searchable marketplace of skills indexed from GitHub repositories. Each skill is identified by its repository path (e.g., `/anthropics/skills`) and skill name.
...
You can interact with the registry directly from your terminal using the `grtl` CLI. No configuration needed — most commands work without authentication.
...
grtl skills search "react testing"
...
grtl skills install /anthropics/skills

# Install a specific skill by name
grtl skills install /anthropics/skills pdf
...
grtl skills list --claude
grtl skills list --cursor
...
GenRTL vets every skill in the registry before it reaches you — both through automated scanning and community-driven quality signals.
...
GenRTL automatically scans skills for potential security issues before they appear in the registry:
...
Use the `grtl` CLI to install, search, generate, and remove skills. See the [CLI reference](/clients/cli#skills) for all commands.

```bash
# Quick start
npx grtl skills search pdf
npx grtl skills install /anthropics/skills pdf
```
...
<CardGroup cols={2}>
  <Card title="CLI Reference" icon="terminal" href="/clients/cli">
    All grtl commands for installing, searching, and generating skills
  </Card>
  <Card title="Browse Skills" icon="magnifying-glass" href="https://genrtl.com/skills">
