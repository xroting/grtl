...
the relevant Azure blade or GenRTL card (crop out the surrounding Azure
...
**Where**: Azure Portal → App registrations → **GenRTL** → **Overview**.
...
**Where**: Azure Portal → App registrations → GenRTL → **Certificates &
secrets** → **Client secrets** tab → **+ New client secret**.

**What to capture**: the *Add a client secret* side panel showing the
description field filled in (for example, `GenRTL on-prem`) and an
...
**Where**: Azure Portal → App registrations → GenRTL → **App roles**.

**What to capture**: the App roles list with both roles created and
**Enabled** = true:

- `GenRTL Admin` / Value: `GenRTL.Admin`
- `GenRTL Member` / Value: `GenRTL.Member`
...
**Where**: Enterprise applications → GenRTL → **Users and groups** →
**+ Add user/group**.

**What to capture**: the *Add Assignment* page with:

- **Users**: one user selected (your own name is fine)
- **Select a role**: **GenRTL Admin** (or Member, either is fine)
...
## 07-genrtl-settings-card.png

**Where**: GenRTL dashboard → **Workspace Settings** → scroll to the
...
- **Admin role value**: `GenRTL.Admin`
- **Member role value**: `GenRTL.Member`
...
- **Consistency**: try to keep the same browser chrome, zoom level, and
  theme across the Azure shots and across the GenRTL shots so the
