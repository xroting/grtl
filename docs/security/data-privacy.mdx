...
How GenRTL handles privacy, retrieval scope, and data.

## Query Privacy

**Your original prompts and code stay with your AI assistant.**

When you use GenRTL through an MCP client, the AI assistant (not the user directly) formulates search queries to retrieve relevant documentation. Here is what happens:
...
3. Only these formulated queries are sent to the GenRTL API — your full prompt, source code, and conversation history are never transmitted
...
### What is sent to the GenRTL API
...
<Note>
  ...
  remain with your AI assistant and are never sent to GenRTL.
</Note>
...
MCP-formulated queries are passed to LLMs to rerank and select the most relevant documentation for your request. GenRTL uses well-known, trusted LLM providers for this purpose — including **OpenAI**, **Google Gemini**, and **Anthropic**.
...
Contact our sales team at [genrtl.com](https://genrtl.com/contact) for Enterprise and on-premise plan details.

## Customizing What Is Shared

The GenRTL MCP server is [open source](https://github.com/upstash/genrtl). If you want full control over what is sent as the `query` parameter, you can:

1. Fork the [GenRTL MCP repository](https://github.com/upstash/genrtl)
2. Edit the tool input descriptions in [`packages/mcp/src/index.ts`](https://github.com/upstash/genrtl/blob/master/packages/mcp/src/index.ts) — these descriptions instruct the AI assistant on how to formulate the query
...
**GenRTL does not store your source files.**
