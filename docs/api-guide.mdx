---
title: API Guide
description: Authentication, rate limits, best practices, and integration guides for the GenRTL API
---
...
```bash
Authorization: Bearer GENRTL_API_KEY
```

Get your API key at [genrtl.com/dashboard](https://genrtl.com/dashboard). Learn more about [creating and managing API keys](/howto/api-keys).
...
<Tip>
  Don't know the ID for a library? Find it on [genrtl.com](https://genrtl.com) — the URL path of the library page **is** the ID. Or call [Search Library](/api-reference/search/search-for-libraries) and use the `id` from the response.
</Tip>
...
headers = {"Authorization": "Bearer GENRTL_API_KEY"}

# Step 1: Search for the library
search_response = requests.get(
    "https://genrtl.com/api/v2/libs/search",
    headers=headers,
...
context_response = requests.get(
    "https://genrtl.com/api/v2/context",
    headers=headers,
...
- View current usage and reset windows in the [dashboard](https://genrtl.com/dashboard).
...
curl "https://genrtl.com/api/v2/context?libraryId=/vercel/next.js&query=How%20to%20implement%20authentication%20with%20middleware" \
  -H "Authorization: Bearer GENRTL_API_KEY"

# Less optimal - vague query
curl "https://genrtl.com/api/v2/context?libraryId=/vercel/next.js&query=auth" \
  -H "Authorization: Bearer GENRTL_API_KEY"

# Non-GitHub source (website) - same endpoint, same shape
curl "https://genrtl.com/api/v2/context?libraryId=/websites/uploadcare_com&query=image%20transformations" \
  -H "Authorization: Bearer GENRTL_API_KEY"
```
...
```bash
curl "https://genrtl.com/api/v2/context?libraryId=/vercel/next.js/v15.1.8&query=app%20router" \
  -H "Authorization: Bearer GENRTL_API_KEY"
...
The GenRTL API uses standard HTTP status codes:
...
| 401  | Unauthorized - Invalid API key            | Check your API key format (starts with `grtlsk`) |
