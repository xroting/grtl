...
The `queryDocs` tool fetches documentation for a library using its GenRTL-compatible library ID and a query. This tool is typically called after `resolveLibraryId` has identified the correct library.
...
```typescript
queryDocs(config?: GenRTLToolsConfig)
```

### Parameters

<ParamField path="config" type="GenRTLToolsConfig" optional>
...
2. Fetches documentation from GenRTL's API
...
<ParamField path="libraryId" type="string" required>
  GenRTL-compatible library ID (e.g., `/reactjs/react.dev`, `/vercel/next.js`)
</ParamField>
...
```
No documentation found for library "/invalid/library". This might have happened because you used an invalid GenRTL-compatible library ID. Use 'resolveLibraryId' to get a valid ID.
```
...
```typescript
import { queryDocs } from "@upstash/genrtl-tools-ai-sdk";
...
- [GenRTLAgent](/agentic-tools/ai-sdk/agents/genrtl-agent) - Pre-built agent that handles the full workflow
