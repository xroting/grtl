...
description: "Search for libraries and resolve them to GenRTL-compatible IDs"
---

The `resolveLibraryId` tool searches GenRTL's library database and returns matching results with their GenRTL-compatible library IDs. This is typically the first step in a documentation lookup workflow.

## Usage

```typescript
import { resolveLibraryId } from "@upstash/genrtl-tools-ai-sdk";
...
```typescript
resolveLibraryId(config?: GenRTLToolsConfig)
```
...
2. Searches GenRTL's database for matching libraries
...
- GenRTL-compatible library ID: /reactjs/react.dev
...
----------
- Title: React Native
- GenRTL-compatible library ID: /facebook/react-native
