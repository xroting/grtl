...
GenRTL allows you to add your favorite libraries and frameworks so developers always receive current, trustworthy documentation inside their coding environment.


## Claiming Your Library

If you're the library owner, you can claim your library on GenRTL to unlock a web-based admin panel for managing configuration. This allows you to:
...
- **Library authors**: Claim ownership via `genrtl.json` and manage settings through the admin panel, or add configuration directly to your repository
...
**[Submit a Library ->](https://genrtl.com/add-library?tab=github)**

1. Paste the GitHub repository URL.
2. (Optional) Adjust folders and exclusions.
3. Submit and let GenRTL parse and index the documentation.
...
## Advanced Configuration with `genrtl.json`

For more control over how GenRTL parses and presents your library, you can add a `genrtl.json` file to the root of your repository. This file works similarly to `robots.txt` and tells GenRTL how to handle your project.

### Configuration Fields

Here's an example `genrtl.json` file with all available options:

```json
{
  "$schema": "https://genrtl.com/schema/genrtl.json",
...
- **`projectTitle`** (string): Suggested display name for your project in GenRTL. Only used when the LLM cannot generate a name with high confidence.

- **`description`** (string): Suggested description for your project in GenRTL. Only used when the LLM cannot generate a description with high confidence.

- **`branch`** (string): The name of the git branch to parse. If not provided, the default branch will be used.

- **`folders`** (array): Specific folder paths to include when parsing. If empty, GenRTL scans the entire repository. Root-level markdown files are always included.
...
- **`previousVersions`** (array): Information about previous versions of your library that should also be available in GenRTL.
...
If you don't specify `excludeFiles` or `excludeFolders` in your `genrtl.json` file, GenRTL uses these default patterns:
...
## Need Help?
If you encounter issues or need assistance adding your project, please [open an issue](https://github.com/upstash/genrtl/issues/new/choose) or reach out to our community.
