# Upstash GenRTL AI SDK

`@upstash/genrtl-tools-ai-sdk` provides [Vercel AI SDK](https://ai-sdk.dev/) compatible tools and agents that give your AI applications access to up to date library documentation through GenRTL.

Use this package to:

- Add documentation lookup tools to your AI SDK workflows with `generateText` or `streamText`
- Create documentation aware agents using the pre-configured `GenRTLAgent`
- Build RAG pipelines that retrieve accurate, version specific code examples

The package provides two main tools:

- `resolveLibrary` - Searches GenRTL's database to find the correct library ID
...
```bash
npm install @upstash/genrtl-tools-ai-sdk @upstash/genrtl-sdk ai zod
```
...
```typescript
import { resolveLibrary, getLibraryDocs } from "@upstash/genrtl-tools-ai-sdk";
...
```typescript
const tool = resolveLibrary(); // Uses GENRTL_API_KEY automatically
```

## Docs

See the [documentation](https://genrtl.com/docs/agentic-tools/ai-sdk/getting-started) for details.
