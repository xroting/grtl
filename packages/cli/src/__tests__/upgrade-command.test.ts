...
describe("upgrade command", () => {
  test("reports when grtl is already up to date", async () => {
    checkForUpdates.mockResolvedValue({
      ...
      upgradePlan: {
        displayCommand: "npm install -g @genrtl/grtl@latest",
      },
    });
  ...
  test("explains ephemeral runners instead of trying to self-upgrade", async () => {
    checkForUpdates.mockResolvedValue({
      ...
      installMethod: "npx",
      upgradePlan: {
        displayCommand: "npx @genrtl/grtl@latest <command>",
        canRun: false,
    ...
    expect(
      plainLogOutput().some((line) => line.includes("npx @genrtl/grtl@latest <command>"))
    ).toBe(true);
    expect(spawn).not.toHaveBeenCalled();
  });
  ...
  test("runs the upgrade command with --yes when possible", async () => {
    checkForUpdates.mockResolvedValue({
      ...
      upgradePlan: {
        command: "npm",
        args: ["install", "-g", "@genrtl/grtl@latest"],
        displayCommand: "npm install -g @genrtl/grtl@latest",
        canRun: true,
        needsExplicitVersion: false,
      },
  ...
  test("shows permissions guidance when install method is unknown but command is global npm", async () => {
    ...
    expect(spawn).not.toHaveBeenCalled();
    expect(
      plainLogOutput().some((line) => line.includes("Run npm install -g @genrtl/grtl@latest"))
    ).toBe(true);
  });
});
...
describe("pre-command upgrade notice", () => {
  ...
  test("shows runner-specific guidance for ephemeral installs", async () => {
    ...
    expect(
      plainLogOutput().some((line) => line.includes("Use npx @genrtl/grtl@latest <command>"))
    ).toBe(true);
